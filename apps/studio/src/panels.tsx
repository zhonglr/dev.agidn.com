...
import { COMPONENT_DRAG_MIME, NODE_DRAG_MIME, resolveInsertTarget, resolveMoveTarget, resolveSiblingMove, SAVED_COMPONENT_DRAG_MIME, type StructureDragErrorCode } from "./structure-drag.js";
...
export function PageOutlinePanel() {
  ...
  return (
    <div className="tool-panel outline-panel">
      <label className="tool-search"><span>{t("common.search")}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("outline.filterPlaceholder")} /></label>
      <div className="tree" role="tree" aria-label={t("outline.treeLabel")} ref={treeRef}>
        ...
        {entries.map(({ node, depth, parentId }) => {
          ...
          return (
            <div role="treeitem" tabIndex={-1} draggable data-node-id={node.id} aria-level={depth + 1} aria-selected={selectedNodeId === node.id} aria-expanded={children.length ? isExpanded : undefined} aria-description={t("outline.dragDescription")} className={`tree-row${selectedNodeId === node.id ? " is-selected" : ""}${dropIndicator?.nodeId === node.id ? ` is-drop-${dropIndicator.position}` : ""}`} style={{ paddingLeft: 6 + depth * 14, "--tree-depth": depth } as CSSProperties} key={node.id} onClick={() => selectNode(node.id)} onKeyDown={(event) => handleKey(event, node, parentId)} onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData(NODE_DRAG_MIME, node.id); event.dataTransfer.setData("text/plain", node.id); session.beginNodeDrag(node.id); selectNode(node.id); }} onDragEnd={() => { session.endNodeDrag(); setDropIndicator(undefined); }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDropIndicator(undefined); }} onDragOver={(event) => {
              ...
              const resolution = resolveMoveTarget(document, catalog, sourceNodeId, node.id, event.clientY, { y: rect.top, height: rect.height });
              setDropIndicator(undefined);
              if (resolution.valid) { setDragError(undefined); void moveNode(sourceNodeId, resolution.target); }
              else setDragError(resolution.reason);
            }}>
              {children.length ? <button type="button" className="tree-disclosure" aria-label={t(isExpanded ? "outline.collapseNode" : "outline.expandNode", { node: nodeTitle(node) })} onClick={(event) => { event.stopPropagation(); setExpanded((value) => { const next = new Set(value); isExpanded ? next.delete(node.id) : next.add(node.id); return next; }); }}>{isExpanded ? "▾" : "▸"}</button> : <span className="tree-disclosure-placeholder" />}
        ...
        {normalizedQuery && entries.length === 0 ? <div className="tool-empty">{t("outline.noMatches", { query })} <button type="button" onClick={() => setQuery("")}>{t("common.clearSearch")}</button></div> : null}
        {dragError ? <div className="tree-drag-message" role="status">{translateStructureDragError(t, dragError)}</div> : null}
      </div>
    </div>
  );
}
...
export function InspectorPanel() {
  ...
  if (!node) return <div className="tool-panel inspector inspector-empty"><span className="selection-icon">N</span><strong>{t("inspector.selectNode")}</strong><p>{t("inspector.selectNodeHelp")}</p></div>;
  ...
  return (
    <div className="tool-panel inspector">
      <div className="selection-summary"><span className="selection-icon">{title[0]?.toUpperCase()}</span><div><strong>{title}</strong><small>{node.kind === "component" ? t("common.component") : t("common.layout")}</small></div></div>
      {node.kind === "component" && definition ? <>
        <div className="inspector-section">
          <button type="button" className="inspector-section__title" aria-expanded={contentOpen} onClick={() => setContentOpen((value) => !value)}><span>{contentOpen ? "▾" : "▸"}</span>{t("inspector.content")}</button>
          ...
          {contentOpen && Object.keys(definition.props).length === 0 ? <p className="inspector-muted">{t("inspector.noProps")}</p> : null}
        </div>
        <div className="inspector-section">
          <button type="button" className="inspector-section__title" aria-expanded={appearanceOpen} onClick={() => setAppearanceOpen((value) => !value)}><span>{appearanceOpen ? "▾" : "▸"}</span>{t("inspector.appearance")}</button>
          {appearanceOpen && definition.variants.length ? <label><span>{t("inspector.variant")}</span><select value={node.variant ?? definition.variants[0]} disabled={session.status === "saving"} onChange={(event) => void session.setVariant(node.id, event.target.value)}>{definition.variants.map((variant) => <option key={variant} value={variant}>{displayLabel(definition.variantDisplayNames?.[variant], variant, locale)}</option>)}</select></label> : null}
          {appearanceOpen ? Object.entries(node.tokens ?? {}).map(([name, token]) => <div className="readonly-field" key={name}><span>{displayLabel(undefined, name, locale)}</span><output title={t("inspector.tokenReadOnlyTitle")}>{displayLabel(undefined, token, locale)}</output></div>) : null}
        </div>
        <div className="inspector-note">{t("inspector.componentNote")}</div>
      </> : <div className="inspector-note">{t("inspector.layoutNote")}</div>}
      <div className="inspector-save">{saveOpen ? <><input autoFocus value={savedName} onChange={(event) => setSavedName(event.target.value)} placeholder={title} /><button type="button" disabled={!savedName.trim()} onClick={() => { if (session.saveSelectedComponent(savedName || title)) { setSaveOpen(false); setSavedName(""); } }}>{t("common.save")}</button><button type="button" onClick={() => setSaveOpen(false)}>{t("common.cancel")}</button></> : <button type="button" onClick={() => { setSavedName(title); setSaveOpen(true); }}>{t("components.saveReusable")}</button>}</div>
      {session.error ? <div className="inspector-error" role="alert">{format(session.error)}</div> : null}
    </div>
  );
}
...
export function ProblemsPanel() {
  ...
  return <div className="bottom-panel">
    <div className="problem-row"><span className={`problem-icon ${session.status === "error" ? "warn" : "info"}`}>{session.status === "error" ? "!" : "i"}</span><div><strong>{session.status === "error" ? t("problems.workspaceAttention") : t("problems.workspaceActive")}</strong><small>{session.error ? format(session.error) : t("problems.revisionSynchronized", { revision: session.revision })}</small></div><span>{t("common.workspace")}</span></div>
    <div className="problem-row"><span className="problem-icon info">i</span><div><strong>{t("problems.previewActive")}</strong><small>{t("problems.previewDescription")}</small></div><span>{t("common.preview")}</span></div>
  </div>;
}
...
export function HistoryPanel() {
  ...
  const summary = (entry: (typeof session.history)[number]): string => entry.kind === "commit"
    ...
    : entry.kind === "restore" ? t("history.restoredRevision", { revision: entry.targetRevision })
      : t(entry.kind === "undo" ? "history.undoToRevision" : "history.redoToRevision", { revision: entry.targetRevision });
  const source = (value: "human" | "system" | "mcp"): string => t(value === "human" ? "history.sourceHuman" : value === "system" ? "history.sourceSystem" : "history.sourceMcp");
  return <div className="bottom-panel history-list">
    {pendingRevision !== undefined ? <div className="history-confirm" role="alert"><div><strong>{t("history.restoreRevision", { revision: pendingRevision })}</strong><small>{t("history.restoreCreatesRevision", { revision: session.revision + 1 })}</small></div><button type="button" onClick={() => setPendingRevision(undefined)}>{t("common.cancel")}</button><button type="button" className="is-primary" disabled={session.status === "saving"} onClick={() => void restore()}>{session.status === "saving" ? t("common.restoring") : t("common.restore")}</button></div> : null}
    {[...session.history].reverse().map((entry) => <div className={entry.revision === session.revision ? "is-current" : ""} key={entry.revision}><span className="history-dot" /><strong>{t("common.revision", { revision: entry.revision })}</strong><small>{summary(entry)}</small><time>{entry.revision === session.revision ? t("common.current") : source(entry.source)}</time>{entry.revision !== session.revision ? <button type="button" className="history-restore" onClick={() => setPendingRevision(entry.revision)}>{t("common.restore")}</button> : null}</div>)}
    <div className={session.revision === 0 ? "is-current" : ""}><span className="history-dot" /><strong>{t("common.revision", { revision: 0 })}</strong><small>{t("common.initialDocument")}</small><time>{session.revision === 0 ? t("common.current") : t("common.workspace")}</time>{session.revision !== 0 ? <button type="button" className="history-restore" onClick={() => setPendingRevision(0)}>{t("common.restore")}</button> : null}</div>
    {session.error ? <div className="history-error" role="alert">{format(session.error)}</div> : null}
  </div>;
}
