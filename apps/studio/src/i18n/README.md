...
4. Run `pnpm test`. The i18n contract rejects missing keys, mismatched placeholders, raw English JSX labels and incomplete Golden Catalog display metadata.

Component, property, slot and variant names are project metadata, not Studio chrome. Their translations belong in the Component Registry `displayName`, `valueDisplayNames` and `variantDisplayNames` fields. Theme/plugin names and document content remain external metadata and are not translated by Studio.
