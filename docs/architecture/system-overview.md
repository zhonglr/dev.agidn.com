...
│
├── examples/
│   └── foundation/             V2 Foundation Page、9 个基础组件和最小 Command 示例
│
├── tests/
...
### 5.1 `packages/layout-engine`（核心拖放解析已建立，布局策略继续演进）

该模块已建立首批框架无关能力：Tree Index、Drop Policy、节点/Geometry Drop Resolver、二维 Grid 排序和安全深度校验。当前布局节点定义仍位于 `document-schema`，最终文档规则仍由 `rule-engine` 强制执行。Canvas 重建目标和迁移边界见 [Canvas 拖拽与布局引擎重建设计](./canvas-drag-layout-engine.md)。
...
Section、Container、Stack、Row、Grid、Overlay 的完整 sizing、responsive、overflow 和 anchor policy 仍是后续演进范围。
...
### 7.2 Studio Canvas Runtime

Canvas Runtime 位于 `apps/studio/src/canvas/`，在 Studio React 树中直接挂载
`packages/react-renderer`。它只加载受 Component Registry 管理的产品组件，不执行用户提供
的任意脚本。

```text
apps/studio/src/canvas/
├── CanvasViewport.tsx
...
架构不包含 Preview Host、iframe 或跨窗口协议，见
...
画布使用独立 Canvas Viewport。触控板 pinch、双指平移、指针中心缩放和 Fit 命令只改变 Canvas Surface 与 Interaction Overlay，不缩放 Studio Chrome 和其他面板。详细模型和验收标准见 [Studio Workbench 架构](./studio-workbench.md)。
...
定义 Studio 与 Workspace Server 之间带版本的 HTTP 和未来 WebSocket 消息。所有请求和响应都必须通过运行时 Schema 验证。
...
8. `document-engine`
9. `workspace-server`
10. `react-renderer` 与 Studio Canvas Runtime
11. 人类优先的最简 Studio
12. `context-exporter`
