...
│   ├── layout-engine/          受控布局模型
│   ├── pattern-registry/       页面模式
│   ├── rule-engine/            所有强制规则
...
│   ├── mcp-protocol/           后续 MCP 请求与 Command 协议
│   └── studio-ui/              编辑器自身的 UI 组件
│
├── examples/
│   ├── sample-components/      示例真实组件库
│   ├── sample-project/         示例项目
│   └── sample-pages/           示例页面 Schema
│
├── tests/
│   ├── contracts/              Schema 与协议契约测试
│   ├── fixtures/               测试数据
│   ├── integration/            模块集成测试
│   ├── visual/                 渲染视觉回归
│   └── e2e/                    完整拖拽、保存和导出流程
...
### 4.2 `packages/pattern-registry`

该模块管理经过验证的高层页面模式，而不是单纯的组件集合。
...
│   └── history/
├── state/
│   ├── editor-store.ts         编辑器临时状态
│   ├── document-controller.ts  调用 Command Engine
│   └── preview-store.ts
└── main.tsx
```
...
├── actions.json           可用 Action 和 Data Source
