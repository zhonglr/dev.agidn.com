...
export function handleSetLayoutProperty(context: HandlerContext, command: SetLayoutPropertyCommand): HandlerResult {
  ...
  if (command.value === null) delete (node as unknown as Record<string, unknown>)[command.property];
  else (node as unknown as Record<string, unknown>)[command.property] = command.value;
